...
export interface SafeWalletConfig {
  safeAddress: string;
  chainId: number; // 8453 for Base (default), 42161 for Arbitrum, 11155111 for Sepolia (testnet)
  rpcUrl: string;
}
...
export function getChainIdForVenue(venue: 'SPOT' | 'GMX' | 'HYPERLIQUID'): number {
  // Default to Base (production)
  // Set USE_SEPOLIA=true env var to use Sepolia testnet
  const useTestnet = process.env.USE_SEPOLIA === 'true';
  
  switch (venue) {
    case 'SPOT':
      return useTestnet ? CHAIN_IDS.SEPOLIA : CHAIN_IDS.BASE;
    case 'GMX':
      return CHAIN_IDS.BASE; // GMX coming soon on Base
    case 'HYPERLIQUID':
      return CHAIN_IDS.BASE; // Use Base as default
    default:
      return useTestnet ? CHAIN_IDS.SEPOLIA : CHAIN_IDS.BASE;
  }
}
